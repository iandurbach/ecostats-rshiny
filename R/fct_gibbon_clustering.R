...
format_selected_groups <- function(selected_candidates, detections, notes_prefix = "auto") {
  ...
  pieces <- lapply(seq_len(nrow(selected_candidates)), function(i) {
    ...
    data.frame(
      ...
      rec_id = ids,
      grouping_method = rep("automated", length(ids)),
      Notes = rep(
        sprintf(
...
group_prepared_gibbon_detections <- function(detections, mics = NULL, solver = "auto",
...
group_gibbon_detections <- function(recordings, mics = NULL, solver = "auto",
  ...
  group_prepared_gibbon_detections(
    prepared$detections,
    prepared$mics,
    solver = solver,
    max_group_span_seconds = max_group_span_seconds,
...
propose_session_groups <- function(session_rows, mics, group_membership, removed_membership,
                                   start_time = NULL, solver = "auto", ...) {
  validate_grouping_inputs(session_rows, mics)
  protected_ids <- unique(c(
    as.character(group_membership$rec_id),
    as.character(removed_membership$rec_id)
  ))
  eligible <- session_rows[!session_rows$rec_id %in% protected_ids, , drop = FALSE]
  if (!is.null(start_time)) {
    start_time <- as.POSIXct(start_time, origin = "1970-01-01", tz = "UTC")
    if (length(start_time) != 1 || is.na(start_time)) {
      stop("Automatic grouping start time must be a valid single datetime.", call. = FALSE)
    }
    eligible <- eligible[eligible$toa >= start_time, , drop = FALSE]
  }
  if ("suspect_bearing" %in% names(eligible)) {
    suspect <- which(!is.na(eligible$suspect_bearing) & eligible$suspect_bearing)
    eligible$bearing[suspect] <- NA_real_
  }
  if (nrow(eligible) < 2 || length(unique(eligible$mic_id)) < 2) {
    return(empty_group_membership())
  }

  result <- group_prepared_gibbon_detections(eligible, mics, solver = solver, ...)
  proposals <- result$group_membership
  if (nrow(proposals) == 0) return(empty_group_membership())

  proposal_times <- merge(
    proposals[, c("group_ID", "rec_id"), drop = FALSE],
    eligible[, c("rec_id", "toa"), drop = FALSE],
    by = "rec_id"
  )
  starts <- stats::aggregate(
    proposal_times$toa,
    by = list(group_ID = proposal_times$group_ID),
    FUN = min
  )
  ordered_ids <- starts$group_ID[order(starts$x, starts$group_ID)]
  id_map <- stats::setNames(seq_along(ordered_ids), ordered_ids)
  proposals$group_ID <- unname(id_map[as.character(proposals$group_ID)])
  proposals[order(proposals$group_ID, match(proposals$rec_id, eligible$rec_id)), , drop = FALSE]
}

remap_proposed_group_ids <- function(proposals, first_group_id) {
  if (nrow(proposals) == 0) return(proposals)
  old_ids <- sort(unique(proposals$group_ID))
  id_map <- stats::setNames(seq_along(old_ids) + as.integer(first_group_id) - 1L, old_ids)
  proposals$group_ID <- unname(id_map[as.character(proposals$group_ID)])
  proposals
}

...
```{r overview-screenshot, echo=FALSE, out.width='80%', fig.align='center', fig.cap='Annotated overview of the loaded app.'}
knitr::include_graphics("images/app-overview-annotated.png")
```
...
The app currently expects PCM WAV files with 16-, 24-, or 32-bit samples. A database must contain the detection table `Gibbon_A` or `GibbonA`, including its `Call_Type` column, plus `Trex_Location`, `Trex_Orientation`, `Trex_Time_Offset`, and `Sound_Acquisition`.
...
4. Optionally choose one value, such as **Female**, from **Call type** to restrict the displayed detections.
5. Click **Select WAV root folder** and choose the folder immediately above the recorder-specific WAV folders.
6. Click **Load data**.
7. Use **Previous/Next cluster** and **Previous/Next session** to reach the data you want to inspect.
8. Select likely matching detections on the timeline.
9. Compare their aligned spectrograms and bearing arrows.
10. Click **Group**, or use **Auto group** and review the proposal.
11. Repeat until the session has been reviewed.
12. Click **Export RData** to save work that can be reloaded later.

## Step 1: load detection and audio data

The app initially looks like this:

```{r empty-app-screenshot, echo=FALSE, out.width='75%', fig.align='center', fig.cap='The app before data are loaded.'}
knitr::include_graphics("images/app-empty.png")
```
...
After the databases are selected, the **Call type** menu is populated with the distinct values found in `Gibbon_A.Call_Type` across those databases. Choose a single value, such as **Female**, to show only detections with that exact call type. Choose **All call types** to remove the filter.

The filter can be changed before or after clicking **Load data**. It updates the timeline and the detection count in the session summary. Manual selection and automatic grouping then operate on the displayed subset. The filter changes the current view and grouping candidates; it does not delete detections from the loaded data.
...
```{r loaded-app-screenshot, echo=FALSE, out.width='75%', fig.align='center', fig.cap='Vocomatcher with the supplied database and WAV data loaded.'}
knitr::include_graphics("images/app-loaded.png")
```
...
- **Missing `Gibbon_A.Call_Type`**: the call-type menu cannot be populated and the database cannot be loaded.
...
| Call type | Show one `Gibbon_A.Call_Type` level, or all levels | Timeline and grouping candidates |
